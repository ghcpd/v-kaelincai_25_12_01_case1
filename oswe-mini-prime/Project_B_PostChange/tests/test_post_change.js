...
const { getActionLabelV2, resetIdempotencyStore } = require('../src/index');
...
test('v2: idempotency ensures same response and prevents duplicate external calls', async () => {
  ...
  // wrap external normalize function to count
  ...
  assert.strictEqual(a, 'MODERN-SUBMIT'); // normalization uppercases

  // call again and ensure idempotency store prevents another external call
...
// Test 4 - retry/backoff (simulated external transient failure) -> we implement naive retries in test harness
...
test('v2: external transient failure triggers eventual success via retries', async () => {
  ...
  // mock service: fails once then succeeds
  const mockSvc = makeMockService({ delayMs: 10, failForAttempts: 1 });

  // implement a simple retry+backoff wrapper for test scenario
...
// Test 5 - timeout & circuit-breaker (simulate slow/overloaded external)
...
test('v2: slow external triggers timeout and circuit breaker path', async () => {
  ...
  // set tight timeout to make call fail
  let threw = false;
  try {
  ...
  // We expect not to throw at library level; external failures are handled and original label remains
  assert.strictEqual(threw, false, 'v2 should not throw due to slow external');

  // simulate circuit open and ensure external not called
  circuit.open = true;
...
test('v2: saga pattern triggers compensation on side-effect failure', async () => {
  ...
  async function doWork() {
    // simulate a side effect that fails
    sideEffectDone = true;
